...
kreirati. U sluèaju prezentacije to je \emph{beamer}.
 \emph{Tijelo} svake prezentacije mora zapoèeti sljedeæim naredbama:\newline
 \begin{itemize}
\item
  \alert{$\backslash begin\{frame\}\rightarrow$} poèetak novog slajda\newline
\pause
\item
  \vdots
...
Temu prezentacije takoðer je potrebno definirati u preambuli\newline
dokumenta. Teme koje oblikuju izgled prezentacije odreðene su svojim imenima. \newline
Pr: \emph{Boadilla, Bergen, Frankfurt, Hannover, Singapore, Berkley,...} \newline
a pozivaju se na sljedeæi naæin:\newline
...
\begin{frame}
 \frametitle{Izgled prezentacije}
Nakon što smo odabrali osnovnu temu možemo promijeniti boju prezentacije\newline
pozivajuæi:\newline
...
\begin{itemize}
\item
Naslov i podnaslov pojedinog slajda dobivaju se pozivajuæi:\newline
...
Ukoliko tekst koji želimo kreirati sadrži liste, nabrajanja\newline
ili dijelove koje ne želimo otkriti odjednom koristit æemo okoline\newline
...
\begin{frame}
 \frametitle{Struktura teksta}
 \begin{itemize}[<+ -| alert@+>]
\item Prva naglašena reèenica
\item Druga naglašena reèenica
\item Treæa naglašena reèenica\newline\newline
\end{itemize}
\uncover<4-> {
Što smo postigli sa \alert{\textbackslash begin\{itemize\}$\left[\langle+ -| alert@\rangle +\right]$....}
}
 \transboxin[duration=0.2]
\end{frame}
...
posebno za liste koje se sastoje od više dijelova:\newline
 ...
 \alert{\textbackslash item$\langle 2 - \rangle$}  Treæi dio\newline
 \alert{\textbackslash end\{itemize\}}\newline
 \alert{\textbackslash item$\langle 5 - \rangle$}  Treæa cjelina\newline
...
\begin{frame}
 \frametitle{Blokovi}
 Ukoliko želimo neki tekst posebno naglasiti, pozvati æemo sljedeæu \emph{okolinu}: \newline
 \alert{$\textbackslash begin\{block\}\{Naslov\}$}\newline
 $\vdots$ \newline
 \alert{$\backslash end\{block\}$}\newline

koja kreira blokove unutar slajdova na sljedeæi naèin....
 \transboxin[duration=0.2]
\end{frame}
...
\end{frame}
\begin{frame}
\only<+>{Ova reèenica pojavljuje se samo na ovom slajdu .}
\uncover<+->{   
Ova ju mijenja na sljedeæem.} 
